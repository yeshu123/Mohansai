...
import { faHome,faEye,faUpload } from '@fortawesome/free-solid-svg-icons';
...
export class Company1Component implements OnInit {
  ...
  LoadCompany() {
    this.api
      ...
      .subscribe((response) => {
        ...
        const finaldata = this.companydata.filter(
  ...
  getDelayDays(item: any) {
    ...
    if (diffDays > 0 && (item.programStatus === 'Submitted' || item.programStatus === 'Approved')) {
      return diffDays;
    } else {
